...
def draw_keypoints_on_3d_graph(plottable_landmarks, AX, landmark_drawing_spec):
                ...
                if arms_and_angles_diff[start_idx]:
                    for arms, diff in arms_and_angles_diff[start_idx].items():
                        if end_idx in arms:
                            ...
                            left = max(left, (abs(diff[0])+abs(diff[1])+abs(diff[2])) / (np.pi))
                ...
                if arms_and_angles_diff[end_idx]:
                    for arms, diff in arms_and_angles_diff[end_idx].items():
                        if start_idx in arms:
                            ...
                            right = max(right, (abs(diff[0])+abs(diff[1])+abs(diff[2])) / (np.pi))
...
def plot_3d_error_graphics(
    ...
    if (
        (not ideal_arms_and_angles)
        or (not plottable_landmarks)
        or (not arms_and_angles)
    ):
    ...
    if connections:
        ...
        for connection in connections:
            ...
            if start_idx in plottable_landmarks and end_idx in plottable_landmarks:
                landmark_pair = [
    ...
    if mp_pose and verbose:
        for idx, pos in plottable_landmarks.items():
